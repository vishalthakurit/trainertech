...
class Admin extends CI_Controller {
	...
	public function create_admin(){
		...
		if ($this->form_validation->run() == TRUE){	
			...
			$created = $this->turnskey_model->create_user($data2,true);//second param auto activates.
			
			if($created){
				$this->session->set_flashdata('success_msg', 'Admin user created with id '.$created['user_id']);
			}
		}
		$data['template'] = 'admin/create_admin';
		$this->load->view('templates/admin_template',$data);
	}
	...
	public function send_registration_email($fname, $lname, $email, $password){
		...
		$html = $this->template_for_registration($fname, $lname, $email, $password);
	...
	public function template_for_registration($fname, $lname, $email, $password){
		$message = '';
		$message .= '<html>
						<body>
